<template>
	<div
		...
		:class="{ 'task-card--blocked': isBlocked }"
		role="button"
	...
	>
		<div
			...
			:style="{ backgroundColor: isBlocked ? 'var(--color-warning)' : categoryColor }"
		></div>
		<div class="flex items-center">
			<div class="flex-1 min-w-0 pr-4" :class="{ 'opacity-50': isBlocked }">
				<h5
				...
				>
					<span v-if="!task.completedDateTime" class="inline-flex items-center mr-1 align-middle">
						<Ban v-if="isBlocked" :size="14" class="text-app-warning" />
				...
				<div v-else class="flex flex-wrap task-details text-sm text-text-secondary">
					...
					<template v-else-if="blockedByNames.length || isPrereqBlocked">
						<p v-if="blockedByNames.length" class="w-full text-left mb-0 mt-0.5 text-app-warning text-xs">
							Blocked by: {{ blockedByNames.join(', ') }}
						</p>
						<p v-if="isPrereqBlocked" class="w-full text-left mb-0 mt-0.5 text-app-warning text-xs">
							Starts {{ new Date(task.prerequisiteDate).toLocaleDateString('en-uk', { day: 'numeric', month: 'short', year: 'numeric' }) }}
						</p>
					</template>
				</div>
			</div>
			<div class="shrink-0">
				<button
					class="complete-btn btn-themed border transition-all"
					:class="isBlocked && !task.completedDateTime
						? 'bg-surface-base border-border-default text-text-secondary opacity-30 cursor-not-allowed'
						: 'bg-surface-hover border-border-default hover:bg-accent hover:text-text-inverse hover:border-accent text-text-secondary'"
					:aria-label="task.completedDateTime ? `Move '${task.name}' back to backlog` : `Mark '${task.name}' as complete`"
					:disabled="isBlocked && !task.completedDateTime"
...
export default {
	...
	computed: {
		...
		isPrereqBlocked() {
			return this.task.prerequisiteDate && new Date(this.task.prerequisiteDate) > new Date()
		},

		isBlocked() {
			return this.task.blocked || this.depBlockedIds.has(this.task.id) || this.isPrereqBlocked
		},
